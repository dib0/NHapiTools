...
namespace NHapiTools.Tests
{
    public class GenericMessageWrapperTests
    {
        ...
        public GenericMessageWrapperTests(ITestOutputHelper output)
        {
            Output = output;
        }

        [Fact(DisplayName = "Segments can be overridden")]
        public void CanOverrideSegments()
        {
            ...
            Assert.Equal(expectedRepetitions, overriddenPid?.AlternatePatientIDRepetitionsUsed);

            var pid1 = ((ADT_A01)originalMessage).PID;
            var pid2 = overriddenPid;

            Assert.Equal(pid1, pid2, new SegmentEqualityComparer());

            Output.WriteLine("Compare PID segments.");
            Output.WriteLine("PID from original message {0} custom PID.", pid1.IsEqual(pid2) ? "is the same as" : "isn't the same as");

            Output.WriteLine("\nDone!");
        }

        [Fact(DisplayName = "Custom ZSegments can be parsed")]
